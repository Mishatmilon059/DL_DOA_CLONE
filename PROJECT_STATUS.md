...
**FNO is a genuinely strong result**: 88% of the pretrained 64-block teacher's mean Pd (0.6235 vs 0.7103), reached from scratch at 8 blocks / 334K params — **~71% of the teacher's 469K params (correction: an earlier draft of this note wrongly said 0.7%)**, i.e. ~29% smaller, not orders of magnitude smaller. FNO's per-block param count is actually much *higher* than a ResNet block's (~41K params/block for `SpectralConv2D`'s dense 12×12×12×12×2 complex weight tensor vs ~7.3K params/block for the teacher's plain 5×5 convs, a ~5.7x difference) — fewer blocks (8 vs 64) still nets out to fewer total params, just not dramatically fewer. This is still the clear candidate to invest further effort in, but the "how small is it" claim needs restating in the thesis as ~29% smaller, not ~99% smaller.
